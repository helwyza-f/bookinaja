...
export default function ResourceDetailLoading() {
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-24 px-4 font-plus-jakarta animate-pulse mt-10">
      {/* --- HEADER SKELETON --- */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b-2 border-slate-100 dark:border-white/5 pb-10 gap-6">
        <div className="space-y-4">
          {/* Breadcrumb back link */}
          <Skeleton className="h-4 w-24 bg-slate-200 dark:bg-slate-800 rounded-md" />
          <div className="flex items-center gap-4">
            {/* Title "Setup Unit" */}
            <Skeleton className="h-12 w-64 md:w-80 bg-slate-200 dark:bg-slate-800 rounded-2xl" />
            {/* Badge Unit Name */}
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          {/* Action Buttons */}
          <Skeleton className="h-14 flex-1 md:w-32 bg-slate-200 dark:bg-slate-800 rounded-2xl" />
          <Skeleton className="h-14 flex-1 md:w-40 bg-blue-600/20 dark:bg-blue-600/20 rounded-2xl" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-8">
          <Card className="rounded-[3rem] border-none shadow-sm bg-white dark:bg-slate-900 overflow-hidden ring-1 ring-slate-100 dark:ring-white/5">
            {/* Tabs Toggle View/Edit */}
            <div className="p-2 bg-slate-50 dark:bg-slate-800/50 flex items-center gap-2">
              <Skeleton className="h-11 flex-1 rounded-2xl bg-white dark:bg-slate-800 shadow-sm" />
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[1, 2, 3, 4].map((i) => (
                <Card
                  key={i}
                  className="rounded-[2.5rem] border-none p-8 bg-white dark:bg-slate-900 ring-1 ring-slate-100 dark:ring-white/5 shadow-sm"
                >
                  <div className="flex items-start justify-between mb-8">
                    ...
                    <div className="flex gap-2">
                      <Skeleton className="h-10 w-10 rounded-xl bg-slate-50 dark:bg-slate-800" />
                      <Skeleton className="h-10 w-10 rounded-xl bg-slate-50 dark:bg-slate-800" />
                    </div>
                  </div>
                  <div className="space-y-4">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[1, 2].map((i) => (
                <Card
                  key={i}
                  className="rounded-[2.5rem] border-none p-7 bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-white/5"
                >
