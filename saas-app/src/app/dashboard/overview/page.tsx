    ...
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-8 md:px-8">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-sm leading-7 text-slate-300">
              Pantau tenant terdaftar, status aktivasi, customer lintas tenant,
              dan transaksi platform dari satu pusat kontrol.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          <Card
            key={card.label}
            className="rounded-[1.75rem] border-slate-200 p-6 shadow-sm"
          >
            ...
            {transactions.map((txn) => (
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-slate-500">
                    {txn.tenant_slug} • {txn.created_at}
                  </div>
                </div>
                <div className="text-right">
      ...
      <Card className="rounded-[2rem] p-6">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {customers.map((customer) => (
            <div
              key={`${customer.id}-${customer.tenant_slug}`}
              className="rounded-2xl border border-slate-200 p-4"
            >
              <div className="font-black">{customer.name}</div>
              <div className="text-sm text-slate-500">
                {customer.tenant_slug} • {customer.phone}
              </div>
              <div className="mt-3 text-xs uppercase tracking-[0.25em] text-slate-500">
                {customer.visits || 0} visits • Rp{" "}
                {(customer.spend || 0).toLocaleString("id-ID")}
              </div>
            </div>
          ))}
        </div>
