...
class OpsController extends ChangeNotifier {
  ...
  int get liveCount => resources.where((r) => r.state == ResourceState.live).length;
  int get idleCount => resources.where((r) => r.state == ResourceState.idle).length;
  int get offCount => resources.where((r) => r.state == ResourceState.off).length;
  int get total => resources.length;
  int get bookingsTodayCount => resources.fold(0, (sum, r) => sum + r.bookingsToday);

  ResourceStatus? get mostBusyResource {
    if (resources.isEmpty) return null;
    final sorted = [...resources]..sort((a, b) => b.bookingsToday.compareTo(a.bookingsToday));
    return sorted.firstWhere((r) => r.bookingsToday > 0, orElse: () => sorted.first);
  }

  ({String resourceName, String customerName, String timeLabel})? get nextBookingSummary {
    final candidates = resources.where((r) => (r.nextBookingCustomerName ?? '').isNotEmpty).toList();
  ...
  List<ResourceStatus> filteredResources({String query = '', ResourceState? stateFilter}) {
  ...
  bool _isRelevantEvent(String type, RealtimeEvent event) {
    if (type.startsWith('booking.') || type.startsWith('payment.') || type.startsWith('session.')) return true;
    if (type.startsWith('resource.') || type.startsWith('device.')) return true;
    final channel = (event.channel ?? '').toLowerCase();
    return channel.contains(':bookings') || channel.contains(':dashboard') || channel.contains(':devices');
  }
  ...
  bool _patchLocal(RealtimeEvent event) {
    ...
    final idx = current.indexWhere((r) =>
        (rid.isNotEmpty && r.resourceId == rid) || (bookingId.isNotEmpty && r.bookingId == bookingId));
  ...
  ResourceStatus? _applyPatch(ResourceStatus item, RealtimeEvent event) {
    ...
    final customerName = _stringOf(summary['customer_name'] ?? summary['customer'], item.liveCustomerName ?? '');
    final resourceName = _stringOf(summary['resource_name'] ?? summary['resource'], item.name);
    final bookingId = _stringOf(event.refs['booking_id'] ?? event.entityId, item.bookingId);
    final endTime = _stringOf(summary['end_time'], item.liveEndsAt ?? '');
    final bookingsToday = _intOf(summary['bookings_today'] ?? summary['bookings_count'], item.bookingsToday);
    final nextCustomer = _stringOf(summary['next_booking_customer_name'], item.nextBookingCustomerName ?? '');
    final nextTime = _stringOf(summary['next_booking_time_label'], item.nextBookingTimeLabel ?? '');
    final nextStatus = _stringOf(summary['next_booking_status'], item.nextBookingStatus ?? '');

    final timelineRaw = summary['today_timeline'];
    final timeline = timelineRaw is List
        ? timelineRaw
            .whereType<Map>()
            .map((e) => ResourceBookingSummary(
                  id: '${e['id'] ?? ''}',
    ...
    final state = _deriveState(item.state, type, statusRaw, endTime, customerName);
    ...
    return ResourceStatus(
      ...
      bookingId: bookingId,
      liveCustomerName: customerName.isNotEmpty ? customerName : item.liveCustomerName,
      liveEndsAt: endTime.isNotEmpty ? endTime : item.liveEndsAt,
      liveRemainingMinutes: endTime.isNotEmpty ? _remainingMinutes(endTime) : item.liveRemainingMinutes,
      bookingsToday: bookingsToday,
      liveCountForResource: state == ResourceState.live ? 1 : 0,
      nextBookingCustomerName: nextCustomer.isNotEmpty ? nextCustomer : item.nextBookingCustomerName,
      nextBookingTimeLabel: nextTime.isNotEmpty ? nextTime : item.nextBookingTimeLabel,
      nextBookingStatus: nextStatus.isNotEmpty ? nextStatus : item.nextBookingStatus,
  ...
  ResourceState _deriveState(
    ...
    if (type.startsWith('device.') || statusRaw == 'inactive' || statusRaw == 'maintenance' || statusRaw == 'archived' || statusRaw == 'disabled') {
      return ResourceState.off;
    }
    if (type.startsWith('session.') || statusRaw == 'active' || statusRaw == 'ongoing' || customerName.isNotEmpty || endTime.isNotEmpty) {
  ...
  String? _buildNote(ResourceState state, String customerName, String endTime, String? fallback) {
    if (state == ResourceState.live) {
      ...
      if (remaining > 0) {
        parts.add(remaining < 60 ? 'sisa ${remaining}m' : 'sisa ${remaining ~/ 60}j ${remaining % 60}m');
      }
