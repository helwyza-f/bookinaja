...
class _PaymentGatewaySettingsScreenState extends State<PaymentGatewaySettingsScreen> {
  ...
  Future<void> _delete() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'Metode pembayaran online akan nonaktif sampai gateway di-setup ulang.',
    ...
    if (ok != true) return;
    try {
      await _repo.deletePaymentGateway();
      if (!mounted) return;
      BkToast.success(context, 'Gateway dihapus');
      _load();
    } catch (e) {
      if (!mounted) return;
                  ...
                  children: [
                    ...
                    FilledButton(
                      ...
                      child: _saving
                          ...
                          : const Text('Simpan kredensial', style: TextStyle(fontWeight: FontWeight.w700)),
                    ),
                    if (_gw.configured) ...[
                      const SizedBox(height: 10),
                      OutlinedButton.icon(
  ...
  Widget _statusCard() {
    final configured = _gw.configured;
    final color = configured ? BK.live : BK.pend;
    final soft = configured ? BK.liveSoft : BK.pendSoft;
    return BKCard(
      child: Row(children: [
        Container(
          width: 44, height: 44,
          decoration: BoxDecoration(color: soft, borderRadius: BorderRadius.circular(12)),
          child: Icon(configured ? Icons.verified_rounded : Icons.vpn_key_outlined, color: color),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(configured ? 'Gateway aktif' : 'Belum di-setup',
                style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14.5, color: BK.ink)),
            const SizedBox(height: 2),
            Text(
              configured
                  ? '${_provider == 'xendit' ? 'Xendit' : 'Midtrans'} · ${_environment == 'production' ? 'Produksi' : 'Sandbox'}'
                  : 'Isi kredensial untuk mengaktifkan pembayaran otomatis.',
