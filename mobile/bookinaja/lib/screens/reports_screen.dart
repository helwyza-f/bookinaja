...
class _ReportsViewState extends State<_ReportsView>
  ...
  void initState() {
    ...
    _tab = TabController(
      ...
      vsync: this,
      initialIndex: _periods.indexOf(c.period),
    );
    // Selesai geser → sinkronkan periode aktif (memicu ensureLoaded panel target).
    _tab.addListener(() {
      if (_tab.indexIsChanging) return;
      c.setPeriod(_periods[_tab.index]);
    });
  }
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              controller: _tab,
              children: [for (final p in _periods) _PeriodPanel(period: p)],
            ),
          ),
        ],
      ),
...
/// Segmented tab dengan indikator pill yang MELUNCUR (pola sama seperti POS).
Widget _periodTabBar(TabController controller) {
  return Padding(
    ...
    child: Container(
      ...
      child: TabBar(
        ...
        tabs: [
          for (final p in ReportPeriod.values) Tab(height: 34, text: p.label),
        ],
      ),
    ),
  );
...
/// Panel isi satu periode. Data dari cache controller: kunjungan pertama fetch,
/// berikutnya instan (tanpa skeleton). KeepAlive agar tak rebuild saat digeser.
class _PeriodPanel extends StatefulWidget {
  final ReportPeriod period;
  const _PeriodPanel({required this.period});
  @override
...
class _PeriodPanelState extends State<_PeriodPanel>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<ReportsController>().ensureLoaded(widget.period);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      color: BK.accent,
      onRefresh: () => c.refresh(widget.period),
      child: c
          .stateFor(widget.period)
          .when(
              ...
              children: [
                ...
                StateView(
                  ...
                  action: FilledButton(
                    ...
                    onPressed: () => c.refresh(widget.period),
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(
          'Diterima Rp ${rupiah(b.diterima)} − Biaya Rp ${rupiah(b.biaya)}',
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              isPos ? Icons.shopping_cart_outlined : Icons.event_note_outlined,
              size: 18,
              color: isPos ? BK.pend : BK.accent,
            ),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  t.customer.isEmpty || t.customer == '-'
                      ? (isPos ? 'Order kasir' : 'Booking')
                      : t.customer,
                  maxLines: 1,
                ...
                Text(
                  '${t.resource.isEmpty ? '' : '${t.resource} · '}${_fmtDate(t.date)}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(
                  '${e.category.isEmpty ? '' : '${e.category} · '}${_fmtDate(e.date)}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: const TextStyle(fontSize: 11.5, color: BK.ink3),
                ),
