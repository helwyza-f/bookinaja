...
/// Buat workspace baru — wizard 2 langkah:
///   1. Nama bisnis (+ preview slug live)
///   2. Mode aplikasi (reservasi/kasir/keduanya)
/// Slug, kategori, dan sisa detail dilengkapi belakangan lewat Setup Bisnis
/// (gerbang publikasi). Setelah sukses, workspace langsung dipilih dan masuk
/// dashboard.
...
class _CreateWorkspaceScreenState extends State<CreateWorkspaceScreen> {
            ...
            Expanded(
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: i <= _step ? BK.accent : BK.line,
  ...
  Widget _stepName() {
    return ListView(
      ...
      children: [
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                  colors: [BK.accent, Color(0xFF7AA2FF)],
                  begin: Alignment.topLeft,
                  end: Alignment.bottomRight),
              borderRadius: BorderRadius.circular(22),
              boxShadow: [
                BoxShadow(
                    color: BK.accent.withValues(alpha: .3),
                    blurRadius: 20,
        ...
        const Text('Beri nama bisnismu',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 21, fontWeight: FontWeight.w800, color: BK.ink)),
        const SizedBox(height: 6),
        const Text('Nama ini tampil ke pelanggan dan jadi alamat halamanmu — bisa diubah nanti.',
            textAlign: TextAlign.center,
            style: TextStyle(color: BK.ink3, fontSize: 13.5, height: 1.4)),
        const SizedBox(height: 26),
        const Text('NAMA BISNIS',
            style: TextStyle(
                fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 1.1, color: BK.ink3)),
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(14),
                borderSide: const BorderSide(color: BK.accent, width: 1.6)),
            errorBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(14),
                borderSide: const BorderSide(color: BK.crit)),
            focusedErrorBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(14),
                borderSide: const BorderSide(color: BK.crit, width: 1.6)),
          ),
        ),
                    ...
                    Flexible(
                      child: Text('$_slug.bookinaja.com',
                          ...
                          style: const TextStyle(
                              fontSize: 12.5, fontWeight: FontWeight.w700, color: BK.accent)),
                    ),
                  ]),
                )
  ...
  // ── Step 2: mode aplikasi ───────────────────────────────────────────────────
  Widget _stepMode() {
    return ListView(
      padding: const EdgeInsets.fromLTRB(20, 12, 20, 24),
      children: [
        const SizedBox(height: 6),
        const Text('Pilih cara kerja aplikasi',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 21, fontWeight: FontWeight.w800, color: BK.ink)),
        const SizedBox(height: 6),
        const Text('Setup & langkah persiapan menyesuaikan pilihan ini — bisa diubah nanti di Pengaturan.',
            textAlign: TextAlign.center,
            style: TextStyle(color: BK.ink3, fontSize: 13.5, height: 1.4)),
              ...
              child: RichText(
                text: const TextSpan(
                  ...
                  children: [
                    ...
                    TextSpan(
                        text: 'Setup Bisnis',
                        style: TextStyle(fontWeight: FontWeight.w800, color: BK.ink)),
                    TextSpan(
                        text: ' untuk melengkapi kategori, kontak, resource, dan metode bayar — lalu terbitkan.'),
                  ],
                ),
              ),
            ),
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(
            ...
            height: 38,
            decoration: BoxDecoration(
                color: on ? BK.accent : BK.card2, borderRadius: BorderRadius.circular(10)),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(m.label,
                  style: TextStyle(
                      fontSize: 14, fontWeight: FontWeight.w800, color: on ? BK.accent : BK.ink)),
  ...
  Widget build(BuildContext context) {
    final canNext = _step == 0 ? _hasName : true;
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Workspace baru · ${_step + 1}/$_totalSteps',
      ...
      bottomNavigationBar: SafeArea(
        ...
        child: FilledButton(
          style: FilledButton.styleFrom(
            backgroundColor: canNext ? BK.accent : BK.ink3,
            disabledBackgroundColor: BK.line,
            padding: const EdgeInsets.symmetric(vertical: 16),
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
          ),
          onPressed: (_busy || !canNext) ? null : _next,
          child: _busy
              ? const SizedBox(
                  height: 20,
                  width: 20,
                  child: CircularProgressIndicator(strokeWidth: 2.2, color: Colors.white))
              : Text(_step == 0 ? 'Lanjut' : 'Buat & lanjut setup',
