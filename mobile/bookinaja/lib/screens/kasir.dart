              ...
              Expanded(
                ...
                child: SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    icon: const Icon(Icons.table_restaurant_rounded, size: 18),
                    label: const Text('Buka bon',
                        maxLines: 1, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14.5)),
                  ),
                ),
              ),
              ...
              Expanded(
                ...
                child: SizedBox(
                  ...
                  child: FilledButton.icon(
                    ...
                    label: const Text('Ke pembayaran',
                        maxLines: 1, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14.5)),
                  ),
                ),
              ),
...
Future<void> _openPayment(BuildContext context, PosController ctrl) async {
  ...
  final outcome = await showModalBottomSheet<PosPaymentOutcome>(
    ...
    builder: (_) => ChangeNotifierProvider<PosController>.value(
      value: ctrl,
      child: PosPaymentSheet(total: ctrl.cartTotal, methods: ctrl.paymentMethods),
    ),
  );
  if (outcome == null || !context.mounted) return;
  if (outcome.keptOpen) {
    BkToast.success(context, 'Pembayaran tercatat', subtitle: 'Bon tetap terbuka di Pesanan terbuka.');
  } else {
    // Struk ditampilkan dari context KasirScreen yang stabil setelah sheet tertutup.
