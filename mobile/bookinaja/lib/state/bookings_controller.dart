...
class BookingsController extends ChangeNotifier {
  ...
  // Filter cepat status (single-select). Penelusuran per-tanggal ada di kalender.
  int filter = 0; // 0 Semua · 1 Aktif · 2 DP · 3 Lunas · 4 Batal
  ...
  List<Booking> get filtered {
    ...
    list = switch (filter) {
      1 => list.where((b) => b.status == BookingStatus.live),
      2 => list.where((b) => b.status == BookingStatus.dp),
      3 => list.where((b) => b.status == BookingStatus.paid),
      4 => list.where((b) => b.status == BookingStatus.cancelled),
      _ => list,
    };
