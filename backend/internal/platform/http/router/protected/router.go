...
func Register(r *gin.RouterGroup, cfg routecfg.Config) {
	...
	protected.Use(middleware.AuthMiddleware(cfg.DB))
	{
		...
		platformProtected.Use(middleware.PlatformOnly())
		{
			...
			platformProtected.GET("/payment-gateway", cfg.PlatformHandler.GetPaymentGatewaySetting)
			platformProtected.PATCH("/payment-gateway", cfg.PlatformHandler.UpdatePaymentGatewaySetting)
		...
		adminArea.Use(middleware.AdminOnly())
		{
			...
			ownerArea.Use(middleware.OwnerOnly())
			{
				ownerAdmin := ownerArea.Group("/admin")
				{
					...
					ownerAdmin.POST("/growth/posts", middleware.RequireActiveSubscription(cfg.DB), cfg.TenantHandler.CreateTenantPost)
				...
				settingsGroup := ownerArea.Group("/admin/settings")
				{
					...
					settingsGroup.POST("/promos", middleware.RequireActiveSubscription(cfg.DB), cfg.PromoHandler.Create)
			...
			resources := adminArea.Group("/resources-all")
			{
				...
				resources.POST("", middleware.RequireActiveSubscription(cfg.DB), middleware.RequirePermission(tenant.PermissionResourcesCreate), cfg.ResourceHandler.Create)
				...
				resources.POST("/:id/items", middleware.RequireActiveSubscription(cfg.DB), middleware.RequirePermission(tenant.PermissionResourcesUpdate), cfg.ResourceHandler.AddItem)
			...
			salesOrders := adminArea.Group("/sales-orders")
			{
				...
				salesOrders.POST("", middleware.RequireTransactionsAllowed(cfg.DB), middleware.RequirePermission(tenant.PermissionPosOrderAdd), cfg.SalesHandler.Create)
				salesOrders.POST("/menu", middleware.RequireTransactionsAllowed(cfg.DB), middleware.RequirePermission(tenant.PermissionPosOrderAdd), cfg.SalesHandler.CreateMenuOrder)
				salesOrders.POST("/direct", middleware.RequireTransactionsAllowed(cfg.DB), middleware.RequirePermission(tenant.PermissionPosOrderAdd), cfg.SalesHandler.CreateDirectOrder)
			...
			bookings := adminArea.Group("/bookings")
			{
				...
				bookings.PUT("/:id/reschedule", middleware.RequirePermission(tenant.PermissionBookingsUpdate), cfg.ReservationHandler.Reschedule)
				bookings.PUT("/:id/note", middleware.RequirePermission(tenant.PermissionBookingsUpdate), cfg.ReservationHandler.UpdateNote)
				...
				bookings.POST("/manual", middleware.RequireTransactionsAllowed(cfg.DB), middleware.RequirePermission(tenant.PermissionBookingsCreate), cfg.ReservationHandler.Create)
			...
			fnbGroup := adminArea.Group("/fnb")
			{
				...
				fnbGroup.POST("", middleware.RequireActiveSubscription(cfg.DB), middleware.RequirePermission(tenant.PermissionFnbCreate), cfg.FnbHandler.CreateItem)
