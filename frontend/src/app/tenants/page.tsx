              ...
              <div className="space-y-3">
                {explorerGrid.slice(0, 6).map((tenant, index) => (
                  <FeedRowCard
                    ...
                    cardVariant="feed-row"
                    positionIndex={index}
                    onVisible={() => markImpression(tenant, "discover", "quick-explore", "feed-row", index)}
                  />
                ))}
              </div>
            </section>
    ...
    <section className="space-y-4">
      ...
      />
        ...
        {section.items.map((tenant, index) => (
          <div key={tenant.id} className="w-[232px] min-w-[232px] flex-none md:w-[260px] md:min-w-[260px]">
            <CompactDiscoveryCard
              tenant={tenant}
              surface="discover"
...
function CompactDiscoveryCard({
  ...
  return (
    <Card className={cn("group h-full overflow-hidden rounded-[1.55rem] border shadow-[0_14px_30px_rgba(15,23,42,0.06)] transition-all hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(15,23,42,0.09)]", bookinajaDiscoveryTheme.cardBorder, bookinajaDiscoveryTheme.cardBg)}>
      <CardContent className="flex h-full flex-col p-0">
        ...
        <div className="flex flex-1 flex-col p-3.5">
          <div className="flex items-start justify-between gap-3">
            <div
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-white"
              style={tenant.primary_color ? { backgroundColor: tenant.primary_color } : undefined}
            >
              {tenant.logo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
          ...
          <h3 className="mt-3 line-clamp-2 text-base font-black uppercase tracking-[-0.03em] text-slate-950">
            {tenant.name}
          </h3>
          <p className={cn("mt-2 line-clamp-2 text-sm leading-6", bookinajaDiscoveryTheme.mutedText)}>
            {tenant.discovery_headline || tenant.tagline || tenant.about_us}
          </p>
          ...
          <div className="mt-3 grid gap-2 rounded-2xl bg-white/80 p-3 text-[11px] font-medium text-slate-500 ring-1 ring-blue-100">
          ...
          <p className={cn("mt-3 text-sm leading-6", bookinajaDiscoveryTheme.mutedText)}>
            {tenant.highlight_copy || tenant.availability_hint || tenant.discovery_subheadline}
          </p>

          <Button
            asChild
            className={cn("mt-4 h-10 rounded-2xl text-sm font-semibold shadow-sm hover:opacity-95", bookinajaDiscoveryTheme.accentStrong)}
          >
            <a
              href={getTenantUrl(tenant.slug)}
              onClick={() =>
...
function FeedRowCard({
  ...
  return (
    <Card className="overflow-hidden rounded-[1.6rem] border border-blue-100 bg-white shadow-[0_12px_28px_rgba(15,23,42,0.05)]">
      <CardContent className="p-0">
        <div className="flex gap-3 p-3">
          <div
            className="h-24 w-24 shrink-0 rounded-[1.2rem] bg-cover bg-center"
            style={{
              backgroundImage: tenant.featured_image_url || tenant.banner_url
                ? `url(${tenant.featured_image_url || tenant.banner_url})`
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(tenant.discovery_tags || tenant.discovery_badges || []).slice(0, 2).map((tag) => (
                <span
                  key={tag}
        ...
        <div className="border-t border-slate-100 px-3 pb-3 pt-2">
          <Button
            asChild
            className={cn("h-10 w-full rounded-2xl text-sm font-semibold shadow-sm hover:opacity-95", bookinajaDiscoveryTheme.accentStrong)}
          >
            <a
              href={getTenantUrl(tenant.slug)}
              onClick={() =>
