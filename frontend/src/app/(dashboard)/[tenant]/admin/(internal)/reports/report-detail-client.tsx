  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <Card className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-950">
        ...
        <div className={kind === "ledger" ? "hidden overflow-x-auto md:block" : "overflow-x-auto"}>
...
function LedgerMobileList({
    ...
    return (
      <div className="space-y-3 p-3 md:hidden">
        {Array.from({ length: 4 }).map((_, index) => (
          <div
            key={index}
            className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.03]"
          >
  ...
  if (rows.length === 0) {
    return (
      <div className="p-4 md:hidden">
        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-8 text-center dark:border-white/10 dark:bg-white/[0.03]">
          <p className="text-sm font-semibold text-slate-900 dark:text-white">
            Belum ada mutasi saldo
          </p>
          <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">
            Dana Midtrans dari booking atau POS akan muncul di sini setelah pembayaran selesai.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-3 p-3 md:hidden">
      {rows.map((row, index) => {
        const isCredit = String(row.jenis_mutasi || "").toLowerCase().includes("masuk");
        const status = String(row.status_dana || "-");
        return (
          <article
            key={`${String(row.nomor_pembayaran || "mutasi")}-${index}`}
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <MobileLedgerMetric
                label="Masuk saldo"
                value={formatIDR(Number(row.masuk_saldo || 0))}
                strong
                tone={isCredit ? "credit" : "neutral"}
              />
              <MobileLedgerMetric
                label="Saldo tenant"
                value={formatIDR(Number(row.saldo_tenant || 0))}
              />
              <MobileLedgerMetric
                label="Nilai transaksi"
                value={formatIDR(Number(row.nilai_transaksi || 0))}
              />
              <MobileLedgerMetric
                label="Biaya"
                value={formatIDR(Number(row.biaya || 0))}
              />
            </div>

            <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-3 text-[11px] font-medium text-slate-500 dark:border-white/10 dark:text-slate-400">
              <span>{String(row.metode || "-")}</span>
              <span className="text-slate-300 dark:text-slate-600">/</span>
              <span>{String(row.jenis_mutasi || "-")}</span>
              {String(row.nomor_transaksi || "-") !== "-" ? (
                <>
                  <span className="text-slate-300 dark:text-slate-600">/</span>
                  <span className="truncate">{String(row.nomor_transaksi)}</span>
                </>
              ) : null}
            </div>
          </article>
